...
```{r, output_files, include='TRUE'}
 ...
 pred_train<-read.table("GEBV_GRS.txt")
 pred_test<-read.table("testing.pred.pl",header = T)
...
The Pearson correlation between observed and predicted phenotype in the training set was `r round(cor(pred_train$V3,y_real$V1[1:length(pred_train$V3)]), 2)`.
...
```{r, predictive_accuracy_train, include='TRUE'}
 ...
 plot(pred_train$V3,y_real$V1[1:length(pred_train$V3)],xlab="yHat",ylab="Real y")
 plot(pred_train$V3,pred_train$V2,ylab="yHat",xlab="Observed y")
 lm.fit<-lm(pred_train$V2~pred_train$V3)
 abline(lm.fit,col=2)
```
...
```{r, include='FALSE'}
	  ...
	  ytest<-read.table(file='testing_regression.txt',header=F)


```

The Pearson correlation between observed and predicted phenotype in the testing set was `r round(cor(pred_test$GEBV_GRS,y_real$V1[(length(pred_train$V3)+1):length(y_real$V1)]), 2)`.
