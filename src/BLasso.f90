...
real*8, allocatable:: t(:),ln_t(:),t_c(:),snp(:),snp2(:),epig(:,:)
   real*8, allocatable:: d(:),kernelinv(:,:) !diagonal de A
...
!OTROS (contadores,semillas, formatos, etc)
	...
	character (len=60) ::fdatos,fpedigri,fpar
	real*8 :: tiempo1,tiempo2,tiempo
	real*8::den1,den0,ratio,sc_S,sc_ep,sc_p,sc_e,sc_r,sc_t0,sc_t1
...
integer:: tot,acc,cont,xx(n_efectos) !nf=numero de efectos fijos + aleatorios + covariables
...
scale=lambda**2
do j=1,n_cov
    if (abs(sol(j)).lt.0.000001) sol(j)=0.000001d0 !print *,'beta ',k,' lower than 1.E-08'
...
!   do j=1,n_datos
!      sc_S=sc_S+sol(i)*kernelinv(i,j)*sol(j)
!  enddo
enddo
...
subroutine predictions
 ...
 open (78,file='testing_regression.data', form='formatted', status='old')
 ...
 write (79,*) 'ANIMAL  ACTUAL_PHENTP EST_PHENTP'

 do n=1,n_yng
    read (78,*) y_real,id_yng,yng_snp(1:n_cov)
    y_est=muf 
    do i=1,n_cov
...
DO i=1,n_datos 
  ...
  print *,'****LECTURA DEL ARCHIVO DE DATOS****'
  DO i=1,3
    print *,''
...
!PAUSE
CLOSE (10)
!SE GUARDAN LOS PUNTOS DE CENSURA EN CASO DE DATOS CENSURADOS
     DO i=1,n_datos
        y1(i)=y(i)
     END DO

    !ALGORITMO 'DATA AUGMENTATION' EN CASO DE DATOS FALTANTES
   k=0;n_df=0
   IF (df.eq.1) THEN
     DO i=1,n_datos
